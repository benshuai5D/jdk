...
class G1PostEvacuateCollectionSetCleanupTask2::EagerlyReclaimHumongousObjectsTask : public G1AbstractSubTask {
  ...
  size_t _bytes_freed;
  uint* _reclaimed_region_count;

public:
  EagerlyReclaimHumongousObjectsTask(uint* reclaimed_region_count) :
    G1AbstractSubTask(G1GCPhaseTimes::EagerlyReclaimHumongousObjects),
    _humongous_regions_reclaimed(0),
    _bytes_freed(0),
    _reclaimed_region_count(reclaimed_region_count) { }
  ...
  void do_work(uint worker_id) override {
    ...
    *_reclaimed_region_count = _humongous_regions_reclaimed;
  }
};
